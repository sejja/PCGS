...
def training(args_param, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    lmbda_list = args_param.lmbda_list
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        lmbda_idx = np.random.choice(len(lmbda_list))
        lmbda = lmbda_list[lmbda_idx] * 1e-3
        args_param.lmbda = lmbda
        gaussians.current_lmbda = lmbda
        gaussians.mlp_grid.current_lmbda = lmbda
        gaussians.current_lmbda_idx = lmbda_idx
        gaussians.mlp_grid.cli = lmbda_idx
        ...
        if iteration % 1000 == 0 and bit_per_param is not None:
            ...
            logger.info("\n----------------------------------------------------------------------------------------")
            logger.info("\n-----[ITER {}] bits info: lmbda_idx={}, lmbda={}-----".format(iteration, gaussians.current_lmbda_idx, gaussians.current_lmbda))
        ...
        with torch.no_grad():
            ...
            if iteration in testing_iterations:
                logger.info("\n ----------- ------------ ------------- ----------- ------------ ------------- ----------- ------------ -------------")
                for lmbda_idx in range(len(lmbda_list)):
                    ...
                    with torch.no_grad():
                        ...
                        mask_1_rate, mask_size_bit, mask_size_MB, mask_numel = get_binary_vxl_size(
                            binary_grid_masks_anchor + 0.0)  # [0, 1] -> [-1, 1]
                    logger.info("\n-----[ITER {}] bits info: 1_rate_mask={}, mask_numel={}, mask_size_MB={}-----".format(iteration, mask_1_rate, mask_numel, mask_size_MB))
                logger.info("\n ----------- ------------ ------------- ----------- ------------ ------------- ----------- ------------ -------------")
...
def training_report(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, wandb=None, logger=None, pre_path_name=''):
    ...
    if iteration in testing_iterations:
        ...
        if iteration in testing_iterations:
            ...
            if iteration == testing_iterations[-1] and run_codec:
                if scene.gaussians.current_lmbda_idx == 0:  # encode or save model
                    bit_stream_path = os.path.join(pre_path_name, 'bitstreams')
                    os.makedirs(os.path.join(pre_path_name, 'decoded_model'), exist_ok=True)
                    os.makedirs(bit_stream_path, exist_ok=True)

                    log_info_list = scene.gaussians.conduct_encoding(pre_path_name=bit_stream_path)
                    for log_info in log_info_list:
                        logger.info(log_info)

                bit_stream_path = os.path.join(pre_path_name, 'bitstreams')
                log_info = scene.gaussians.conduct_decoding(pre_path_name=bit_stream_path, decode_until=scene.gaussians.current_lmbda_idx+1)
                logger.info(log_info)

                save_decoded_path = os.path.join(pre_path_name, 'decoded_model', f'decoded_model_ss{scene.gaussians.current_lmbda_idx}.pkl')
                os.makedirs(os.path.join(pre_path_name, 'decoded_model'), exist_ok=True)
                torch.save(scene.gaussians.state_dict(), save_decoded_path)
                scene.gaussians.load_state_dict(torch.load(save_decoded_path))
...
def render_set(model_path, ss, name, iteration, views, gaussians, pipeline, background):
    name = name + "_ss" + str(ss)

    lmbda_list_len = len(os.listdir(os.path.join(model_path, 'decoded_model')))
    save_decoded_path = os.path.join(model_path, 'decoded_model', f'decoded_model_ss{ss}.pkl')
    decoded_weight = torch.load(save_decoded_path)

    NNN = decoded_weight['_anchor'].shape[0]
    NNN2 = decoded_weight['_rotation'].shape[0]
    gaussians._anchor = nn.Parameter(torch.zeros(NNN, 3, device="cuda"))
    gaussians._offset = nn.Parameter(torch.zeros(NNN, 10, 3, device="cuda"))
    gaussians._mask = nn.Parameter(torch.zeros(NNN, 10, lmbda_list_len, device="cuda"))
    gaussians._anchor_feat = nn.Parameter(torch.zeros(NNN, 50, device="cuda"))
    gaussians._scaling = nn.Parameter(torch.zeros(NNN, 6, device="cuda"))
    gaussians._rotation = nn.Parameter(torch.zeros(NNN2, 4, device="cuda"))
    gaussians._opacity = nn.Parameter(torch.zeros(NNN2, 1, device="cuda"))

    gaussians.load_state_dict(decoded_weight)
    gaussians.current_lmbda_idx = ss
...
def render_sets(args_param, dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train=True, skip_test=False, wandb=None, tb_writer=None, dataset_name=None, logger=None, x_bound_min=None, x_bound_max=None):
    ...
    with torch.no_grad():
        ...
        if not skip_test:
            for ss in range(len(lmbda_list)):
                t_test_list, visible_count = render_set(dataset.model_path, ss, "test", args_param.iterations, scene.getTestCameras(), gaussians, pipeline, background)
...
def evaluate(model_paths, visible_count=None, wandb=None, tb_writer=None, dataset_name=None, logger=None):
    len_lmbda_list = len(os.listdir(os.path.join(model_path, 'decoded_model')))
    for ss in range(len_lmbda_list):
        full_dict = {}
        per_view_dict = {}
        ...
        test_dir = Path(scene_dir) / f"test_ss{ss}"
        ...
        for method in os.listdir(test_dir):
            ...
            logger.info(f"model_paths: \033[1;35m{model_paths}\033[0m  ss{ss}")
        ...
        with open(scene_dir + f"/results_ss{ss}.json", 'w') as fp:
            json.dump(full_dict[scene_dir], fp, indent=True)
        with open(scene_dir + f"/per_view_ss{ss}.json", 'w') as fp:
...
def get_logger(path):
    ...
    fileinfo = logging.FileHandler(os.path.join(path, "output.log"))
