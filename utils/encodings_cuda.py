...
def encoder_trinomial(masks, probs, file_name='tmp.b'):
    # masks: [..., 3]
    # probs: [..., 3]
    assert file_name[-2:] == '.b'
    masks = masks.view(-1, 3)  # [N, 3]
    probs = probs.view(-1, 3)  # [N, 3]
    p_0 = torch.zeros_like(probs[:, 0:1])  # [N, 1]
    p_1 = probs[:, 0:1]  # [N, 1]
    p_2 = p_1 + probs[:, 1:2]  # [N, 1]
    p_3 = torch.ones_like(probs[:, 0:1])  # [N, 1]
    output_cdf = torch.cat([p_0, p_1, p_2, p_3], dim=-1)  # [N, 4]

    sym = torch.argmax(masks, dim=-1).to(torch.int16)  # [N]
    ...
    with open(file_name, 'wb') as fout:
        ...
        fout.write(cnt_bytes)
        fout.write(byte_stream_bytes)
    bit_len = (len(byte_stream_bytes) + len(cnt_bytes)) * 8 + 32
    return bit_len


def decoder_trinomial(probs, file_name='tmp.b', device='cuda'):
    # probs: [..., 3]
    shape_orig = probs.shape
    probs = probs.view(-1, 3) # [N, 3]
    assert file_name[-2:] == '.b'
    ...
    mask = nnf.one_hot(sym_out.long(), num_classes=3).to(dtype=torch.float)  # [N, 3]
    mask = mask.view(shape_orig)
    return mask
